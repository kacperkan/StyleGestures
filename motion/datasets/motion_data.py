...
class MotionDataset(Dataset):
    ...
    def __init__(self, control_data, joint_data, seqlen, n_lookahead, dropout):
        ...
        autoreg = self.concat_sequence(
            self.seqlen, joint_data[:, : n_frames - n_lookahead - 1, :]
        )
        ...
        x_start = seqlen
        new_x = self.concat_sequence(
            1, joint_data[:, x_start : n_frames - n_lookahead, :]
        )
        self.x = new_x
        self.cond = new_cond
    ...
    def concat_sequence(self, seqlen, data):
        ...
        for ii in range(0, seqlen):
            inds[:, ii] = np.transpose(
                rng[ii : (n_timesteps - (seqlen - ii - 1))]
            )
