...
class Locomotion_rot:
    def __init__(self, hparams, is_training):
        ...
        # load scalers.
        self.input_scaler = jl.load(
            os.path.join(data_root, "input_scaler.sav")
        )
        self.output_scaler = jl.load(
            os.path.join(data_root, "output_scaler.sav")
        )
        self.data_pipe = jl.load(
            os.path.join(
                data_root,
                "data_pipe_" + str(hparams.Data.framerate) + "fps.sav",
            )
        )
        ...
        if is_training:
            ...
            train_input = np.load(
                os.path.join(
                    data_root,
                    "train_input_" + str(hparams.Data.framerate) + "fps.npz",
                )
            )["clips"].astype(np.float32)
            train_output = np.load(
                os.path.join(
                    data_root,
                    "train_output_" + str(hparams.Data.framerate) + "fps.npz",
                )
            )["clips"].astype(np.float32)
            val_input = np.load(
                os.path.join(
                    data_root,
                    "val_input_" + str(hparams.Data.framerate) + "fps.npz",
                )
            )["clips"].astype(np.float32)
            val_output = np.load(
                os.path.join(
                    data_root,
                    "val_output_" + str(hparams.Data.framerate) + "fps.npz",
                )
            )["clips"].astype(np.float32)
            ...
            test_input = np.load(
                os.path.join(
                    data_root,
                    "dev_input_" + str(hparams.Data.framerate) + "fps.npz",
                )
            )["clips"].astype(np.float32)
        else:
            ...
            test_input = np.load(
                os.path.join(
                    data_root,
                    "test_input_" + str(hparams.Data.framerate) + "fps.npz",
                )
            )["clips"].astype(np.float32)
        ...
        self.n_cond_channels = (
            self.n_x_channels * hparams.Data.seqlen
            + test_input.shape[2]
            * (hparams.Data.seqlen + 1 + hparams.Data.n_lookahead)
        )
        test_output = np.zeros(
            (test_input.shape[0], test_input.shape[1], self.n_x_channels)
        ).astype(np.float32)
    ...
    def save_animation(self, control_data, motion_data, filename):
        ...
        anim_data = np.concatenate(
            (
                inv_standardize(motion_data, self.output_scaler),
                inv_standardize(control_data, self.input_scaler),
            ),
            axis=2,
        )
