...
class Generator(object):
    ...
    def prepare_cond(self, jt_data, ctrl_data):
        ...
        cond = torch.from_numpy(
            np.expand_dims(
                np.concatenate((jt_data, ctrl_data), axis=1), axis=-1
            )
        )
    ...
    def generate_sample(self, graph, eps_std=1.0, step=0, counter=0):
        ...
        autoreg = np.zeros(
            (nn, seqlen, n_feats), dtype=np.float32
        ...
        for i in range(0, control_all.shape[1] - seqlen - n_lookahead):
            ...
            autoreg = np.concatenate(
                (autoreg[:, 1:, :].copy(), sampled[:, None, :]), axis=1
            )

        # store the generated animations
        self.data.save_animation(
            control_all[:, : (n_timesteps - n_lookahead), :],
            sampled_all,
            os.path.join(
                self.log_dir,
