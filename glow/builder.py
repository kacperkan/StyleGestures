...
def build(x_channels, cond_channels, hparams, is_training):
    ...
    graph, optim, lrschedule, criterion_dict = (
        None,
        None,
        None,
        None,
    ...
    if graph is not None:
        ...
        if is_training:
            if (
                "warm_start" in hparams.Train
                and len(hparams.Train.warm_start) > 0
            ):
        ...
        use_cpu = any(
            [isinstance(d, str) and d.find("cpu") >= 0 for d in devices]
        )
        if use_cpu:
            graph = graph.cpu()
            ...
            if is_training and pre_trained is not None:
                ...
                if hasattr(optim, "state"):
                    ...
                    def move_to(D, device):
                        for k in D:
                            if isinstance(D[k], dict) or isinstance(
                                D[k], defaultdict
                            ):
                                move_to(D[k], device)
