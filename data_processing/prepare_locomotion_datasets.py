...
def extract_features(
    bvh_dir, files, destpath, fps, fullbody=False, smooth_pos=4, smooth_rot=4
):
    p = BVHParser()
    ...
    for f in files:
        ...
        np.savez(
            ff + "_timerev_mirrored.npz", clips=out_data[3 * len(files) + fi]
        )
...
def cut_bvh(filename, timespan, destpath, starttime=0.0, endtime=-1.0):
    ...
    while (starttime + timespan) <= endtime:
        ...
        end_idx = (
            int(np.round((starttime + timespan) / bvh_data.framerate)) + 1
        )
...
def slice_data(data, window_size, overlap):
    ...
    sliced = np.zeros((n_sequences, window_size, data.shape[1])).astype(
        np.float32
    )
...
def import_and_slice(
    files, motion_path, slice_window, slice_overlap, start=0, end=None
):
    ...
    for file in files:
        ...
        # slice dataset
        motion_data = np.load(os.path.join(motion_path, file + ".npz"))[
            "clips"
        ].astype(np.float32)
...
if __name__ == "__main__":
    ...
    copyfile(
        os.path.join(motion_path, "data_pipe.sav"),
        os.path.join(processed_dir, f"data_pipe_{fps}fps.sav"),
    )
    ...
    train_motion, train_ctrl = import_and_slice(
        train_files, motion_path, slice_win_train, window_overlap
    )
    val_motion, val_ctrl = import_and_slice(
        held_out,
        motion_path,
        slice_win_train,
        window_overlap,
    ...
    dev_motion, dev_ctrl = import_and_slice(
        held_out, motion_path, slice_win_test, 0, start=0, end=val_test_split
    )
    test_motion, test_ctrl = import_and_slice(
        held_out, motion_path, slice_win_test, 0, start=val_test_split
    )  # we only use the ctrl when testing. the test motion is used as ground truth
    synth_ctrl = create_synth_test_data(slice_win_test, test_ctrl.shape[2])
    synth_motion = np.zeros(
        (synth_ctrl.shape[0], slice_win_test, test_motion.shape[2])
    ).astype(
        np.float32
    )  # dummy values that will never be used
    ...
    np.savez(
        os.path.join(processed_dir, "output_scaler.npz"),
        stds=m_scaler.scale_,
        means=m_scaler.mean_,
    )
    train_ctrl, c_scaler = fit_and_standardize(train_ctrl)
    np.savez(
        os.path.join(processed_dir, "input_scaler.npz"),
        stds=c_scaler.scale_,
        means=c_scaler.mean_,
    )
    ...
    # import pdb;pdb.set_trace()
    np.savez(
        os.path.join(processed_dir, f"train_output_{fps}fps.npz"),
        clips=train_motion,
    )
    np.savez(
        os.path.join(processed_dir, f"train_input_{fps}fps.npz"),
        clips=train_ctrl,
    )
    np.savez(
        os.path.join(processed_dir, f"val_output_{fps}fps.npz"),
        clips=standardize(val_motion, m_scaler),
    )
    np.savez(
        os.path.join(processed_dir, f"val_input_{fps}fps.npz"),
        clips=standardize(val_ctrl, c_scaler),
    )
    np.savez(
        os.path.join(processed_dir, f"dev_output_{fps}fps.npz"),
        clips=standardize(dev_motion, m_scaler),
    )
    np.savez(
        os.path.join(processed_dir, f"dev_input_{fps}fps.npz"),
        clips=standardize(dev_ctrl, c_scaler),
    )
    np.savez(
        os.path.join(processed_dir, f"test_output_{fps}fps.npz"),
        clips=standardize(concat_test_motion, m_scaler),
    )
    np.savez(
        os.path.join(processed_dir, f"test_input_{fps}fps.npz"),
        clips=standardize(concat_test_ctrl, c_scaler),
    )
