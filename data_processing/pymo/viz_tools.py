...
def draw_stickfigure(
    ...
    for joint in joints_to_draw:
        ...
        children_to_draw = [
            c
            for c in mocap_track.skeleton[joint]["children"]
            if c in joints_to_draw
        ]
...
def draw_stickfigure3d(
    ...
    for joint in joints_to_draw:
        ...
        ax.scatter(
            xs=parent_x, ys=parent_y, zs=parent_z, alpha=0.6, c="b", marker="o"
        )
...
def sketch_move(mocap_track, data=None, ax=None, figsize=(16, 8)):
    ...
    for frame in range(0, data.shape[0], 4):
        ...
        for joint in mocap_track.skeleton.keys():
            children_to_draw = [
                c for c in mocap_track.skeleton[joint]["children"]
            ]
...
def nb_play_mocap_fromurl(
    mocap, mf, frame_time=1 / 30, scale=1, base_url="http://titan:8385"
):
    if mf == "bvh":
        bw = BVHWriter()
    ...
    url = (
        "%s/mocapplayer/player.html?data_url=%s&scale=%f&cz=200&order=xzyi&frame_time=%f"
        % (base_url, filepath, scale, frame_time)
    )
...
def nb_play_mocap(
    ...
    data_template += (
        "start(dataBuffer, metadata, $$CZ$$, $$SCALE$$, $$FRAMETIME$$);"
    )
    ...
    url = "%s?&cz=200&order=xzyi&frame_time=%f&scale=%f" % (
        base_url,
        frame_time,
        scale,
    )
