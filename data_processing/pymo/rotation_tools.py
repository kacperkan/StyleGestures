...
def unroll_1(rots):
    ...
    for ii in range(intv.shape[0]):
        new_ax = -rots[intv[ii, 0] : intv[ii, 1], :] / np.tile(
            angs[intv[ii, 0] : intv[ii, 1], None], (1, 3)
        )
        new_angs = alt_angs[intv[ii, 0] : intv[ii, 1]]
        new_rots[intv[ii, 0] : intv[ii, 1], :] = new_ax * np.tile(
            new_angs[:, None], (1, 3)
        )
...
def offsets_inv(offset, rots, order="XYZ", use_deg=False):
    ...
    q_off = t3d.euler.euler2quat(
        offset[0], offset[1], offset[2], "r" + order.lower()
    )
    q2 = t3d.euler.quat2euler(
        t3d.quaternions.qmult(q0, t3d.quaternions.qinverse(q_off)),
        "r" + order.lower(),
    )
...
def offsets(offset, rots, order="XYZ", use_deg=False):
    ...
    q2 = t3d.euler.quat2euler(
        t3d.quaternions.qmult(q0, q_off), "r" + order.lower()
    )
    # q0=Quaternions.from_euler(rots, order=order.lower())
    # q_off=Quaternions.from_euler(offset, order=order.lower())
    # q2=(q_off*q0).euler(order=order.lower())
...
def euler2expmap(rot, order="XYZ", use_deg=False):
    if use_deg:
        rot = np.deg2rad(rot)
    # print("rot:" + str(rot))
    vec, theta = t3d.euler.euler2axangle(
        rot[0], rot[1], rot[2], "r" + order.lower()
    )
    return vec * theta
...
class Rotation:
    ...
    def _from_euler(self, alpha, beta, gamma, params):
        ...
        self.rotmat = np.transpose(
            t3d.euler.euler2mat(gamma, beta, alpha, axes=order)
        )
