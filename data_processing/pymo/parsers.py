...
class BVHParser:
    ...
    def _to_DataFrame(self):
        ...
        channels = np.asarray(
            [[channel[2] for channel in frame] for frame in frames]
        )
        column_names = ["%s_%s" % (c[0], c[1]) for c in self._motion_channels]

        return pd.DataFrame(
            data=channels, index=time_index, columns=column_names
        )
    ...
    def _parse_joint(self, bvh, token_index):
        ...
        if not end_site:
            channels, token_index, order = self._read_channels(
                bvh, token_index
            )
        ...
        while (
            bvh[token_index][0] == "IDENT" and bvh[token_index][1] == "JOINT"
        ) or (bvh[token_index][0] == "IDENT" and bvh[token_index][1] == "End"):
    ...
    def _parse_hierarchy(self, bvh):
        ...
        offsets, self.current_token = self._read_offset(
