    ...
    def _expmap_to_pos(self, X):
        ...
        for track in X:
            ...
            exp_params = [
                ...
                if (
                    any(p in c for p in ["alpha", "beta", "gamma"])
                    and "Nub" not in c
                )
            ]
            ...
            for joint in track.traverse():
                ...
                if track.root_name == joint:
                    ...
                    # for every frame i, multiply this joint's rotmat to the rotmat of its parent
                    tree_data[joint][0] = np.matmul(
                        rotmats, tree_data[parent][0]
                    )
                    ...
                    # multiply k to the rotmat of the parent for every frame i
                    q = np.matmul(
                        k.reshape(k.shape[0], 1, 3), tree_data[parent][0]
                    )
                ...
                # Create the corresponding columns in the new DataFrame
                pos_df["%s_Xposition" % joint] = pd.Series(
                    data=tree_data[joint][1][:, 0], index=pos_df.index
                )
                pos_df["%s_Yposition" % joint] = pd.Series(
                    data=tree_data[joint][1][:, 1], index=pos_df.index
                )
                pos_df["%s_Zposition" % joint] = pd.Series(
                    data=tree_data[joint][1][:, 2], index=pos_df.index
                )
    ...
    def _to_expmap(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                euler = [
                    [f[1][r1_col], f[1][r2_col], f[1][r3_col]]
                    for f in r.iterrows()
                ]
    ...
    def _expmap_to_euler(self, X):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                euler_rots = [
                    expmap2euler(f, rot_order, True) for f in expmap
                ]  # Convert the exp maps to eulers
...
class Mirror(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            ryp = "%s_Yposition" % track.root_name
            rzp = "%s_Zposition" % track.root_name
            new_df[rxp] = pd.Series(
                data=-signs[0] * euler_df[rxp], index=new_df.index
            )
            new_df[ryp] = pd.Series(
                data=-signs[1] * euler_df[ryp], index=new_df.index
            )
            new_df[rzp] = pd.Series(
                data=-signs[2] * euler_df[rzp], index=new_df.index
            )
            ...
            joints = (
                ...
                for joint in track.skeleton
                if "Nub" not in joint
                and "Left" not in joint
                and "Right" not in joint
            )
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            for joint in joints:
                ...
                new_euler = [
                    euler_reorder(f, rot_order, self.new_order, True)
                    for f in euler
                ]
                ...
                # Create the corresponding columns in the new DataFrame
                new_df[
                    "%s_%srotation" % (joint, self.new_order[0])
                ] = pd.Series(
                    data=[e[0] for e in new_euler], index=new_df.index
                )
                new_df[
                    "%s_%srotation" % (joint, self.new_order[1])
                ] = pd.Series(
                    data=[e[1] for e in new_euler], index=new_df.index
                )
                new_df[
                    "%s_%srotation" % (joint, self.new_order[2])
                ] = pd.Series(
    ...
    def inverse_transform(self, X, copy=None):
        ...
        for track in X:
            ...
            time_index = pd.to_timedelta(
                [f for f in range(track.shape[0])], unit="s"
            )

            new_df = pd.DataFrame(
                data=track,
                index=time_index,
                columns=self.org_mocap_.values.columns,
            )
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            # end of abdolute_translation_deltas
                ...
                forward = (
                    forward
                    / np.sqrt((forward ** 2).sum(axis=-1))[..., np.newaxis]
                )
                ...
                rvelocity = Pivots.from_quaternions(
                ...
                root_pos_x = pd.Series(
                    data=positions[:, 0], index=new_df.index
                )
                root_pos_y = pd.Series(
                    data=positions[:, 1], index=new_df.index
                )
                root_pos_z = pd.Series(
                    data=positions[:, 2], index=new_df.index
                )
                root_pos_x_diff = pd.Series(
                    data=velocity[:, 0], index=new_df.index
                )
                root_pos_z_diff = pd.Series(
                    data=velocity[:, 2], index=new_df.index
                )
                ...
                root_rot_y_diff = pd.Series(
                    data=rvelocity[:, 0], index=new_df.index
                )
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            all_but_root = [
                joint
                for joint in track.skeleton
                if track.root_name not in joint
            ]
            # all_but_root = [joint for joint in track.skeleton]
            for joint in all_but_root:
                new_df["%s_Xposition" % joint] = pd.Series(
                    data=track.values["%s_Xposition" % joint]
                    - projected_root_pos[rxp],
                    index=new_df.index,
                )
                new_df["%s_Yposition" % joint] = pd.Series(
                    data=track.values["%s_Yposition" % joint]
                    - projected_root_pos[ryp],
                    index=new_df.index,
                )
                new_df["%s_Zposition" % joint] = pd.Series(
                    data=track.values["%s_Zposition" % joint]
                    - projected_root_pos[rzp],
                    index=new_df.index,
                )
...
class ConstantsRemover(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.const_values_ = {
            c: X[0].values[c].values[0]
            for c in cols
            if (stds[c] < self.eps).any()
        }
        return self
