...
def extract_hand_pos(bvh_dir, files, destpath, fps):
    ...
    data_pipe = Pipeline(
        [
            ...
            (
                "jtsel",
                JointSelector(["RightHand", "LeftHand"], include_root=False),
            ),
            ("np", Numpyfier()),
        ]
    )
...
def extract_style_features(hand_pos_dir, files, destpath, fps, average_secs):
    ...
    for f in files:
        ...
        np.save(
            os.path.join(os.path.join(destpath, "MG-H"), f) + ".npy",
            h_sm[:, 0],
        )
        ...
        # mirrored
        np.save(
            os.path.join(os.path.join(destpath, "MG-H"), f) + "_mirrored.npy",
            h_sm[:, 1],
        )
        ...
        speed = dr_sm + dl_sm
        np.save(
            os.path.join(os.path.join(destpath, "MG-V"), f) + ".npy", speed
        )
        np.save(
            os.path.join(os.path.join(destpath, "MG-V"), f) + "_mirrored.npy",
            speed,
        )  # same
        ...
        radius = dr_sm + dl_sm
        np.save(
            os.path.join(os.path.join(destpath, "MG-R"), f) + ".npy", radius
        )
        np.save(
            os.path.join(os.path.join(destpath, "MG-R"), f) + "_mirrored.npy",
            radius,
        )
