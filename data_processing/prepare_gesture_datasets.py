...
def cut_audio(filename, timespan, destpath, starttime=0.0, endtime=-1.0):
    ...
    while (starttime + timespan) <= endtime:
        ...
        wav_outfile = os.path.join(
            destpath, out_basename + "_" + str(suffix).zfill(3) + ".wav"
        )
...
def import_data(
    file, motion_path, speech_data, style_path, mirror=False, start=0, end=None
):
    ...
    motion_data = np.load(os.path.join(motion_path, file + suffix + ".npz"))[
        "clips"
    ].astype(np.float32)
    n_motion_feats = motion_data.shape[1]

    speech_data = np.load(os.path.join(speech_path, file + ".npy")).astype(
        np.float32
    )

    if style_path is not None:
        style_data = np.load(
            os.path.join(style_path, file + suffix + ".npy")
        ).astype(np.float32)
...
def import_and_slice(
    ...
    for file in files:
        ...
        # slice dataset
        concat_data, n_motion_feats = import_data(
            file, motion_path, speech_data, style_path, False, start, end
        )
        sliced = slice_data(concat_data, slice_window, slice_overlap)

        if mirror:
            concat_mirr, nmf = import_data(
                file, motion_path, speech_data, style_path, True, start, end
            )
...
if __name__ == "__main__":
    ...
    np.savez(
        os.path.join(processed_dir, f"val_input_{fps}fps.npz"), clips=val_ctrl
    )
    np.savez(
    ...
    np.savez(
        os.path.join(processed_dir, f"dev_input_{fps}fps.npz"), clips=dev_ctrl
    )
    np.savez(
    ...
    if not os.path.exists(dev_vispath):
        ...
        for file in held_out:
            cut_audio(
                os.path.join(audiopath, file + ".wav"),
                test_window_secs,
                dev_vispath,
                starttime=0.0,
