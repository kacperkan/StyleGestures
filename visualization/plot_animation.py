...
def plot_animation(
    clip, parents, filename=None, fps=30, axis_scale=50, elev=45, azim=45
):
    ...
    joints, root_dx, root_dz, root_dr = (
        clip[:, :-3],
        clip[:, -3],
        clip[:, -2],
        clip[:, -1],
    ...
    wframe = ax.plot_wireframe(
        X, Y, Z, rstride=2, cstride=2, color="grey", lw=0.2
    )
    ...
    ani = animation.FuncAnimation(
        fig, animate, np.arange(joints.shape[0]), interval=1000 / fps
    )
